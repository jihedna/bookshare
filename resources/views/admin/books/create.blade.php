                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="author" id="author" value="{{ old('author') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                            @error('author')
                                <span class="text-red-600 text-sm">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="owner_id" id="owner_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->getId() }}" {{ old('owner_id') == $user->getId() ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="categories[]" id="categories" multiple class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                @foreach($categories as $category)
                                    <option value="{{ $category->getId() }}" {{ in_array($category->getId(), old('categories', [])) ? 'selected' : '' }}>
                                        {{ $category->getName() }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <p class="text-gray-500 text-xs mt-1">Format accepté: JPG, PNG, GIF. Taille max: 2Mo.</p>
                            @error('cover')
                                <span class="text-red-600 text-sm">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div>
                        <div class="mb-4">
                            ...
                            <textarea name="description" id="description" rows="12" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('description') }}</textarea>
                            @error('description')
                                <span class="text-red-600 text-sm">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="bg-gray-50 p-4 rounded-md">
                            ...
                            <p class="text-xs text-gray-600 mb-2">
                                Lorsque vous ajoutez un livre avec une description, le système utilisera l'IA pour:
                            </p>
                            <ul class="text-xs text-gray-600 list-disc pl-5 space-y-1">
                                <li>Générer automatiquement un résumé court</li>
